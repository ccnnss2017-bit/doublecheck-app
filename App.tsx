...
const App: React.FC = () => {
  ...
  // 현재 상태를 추적하기 위한 Refs (뒤로가기 로직에서 사용)
  ...
  useEffect(() => { lastStepRef.current = lastStep; }, [lastStep]);
  ...
  // ✨ 스마트폰 하드웨어 뒤로가기 버튼 제어 로직
  useEffect(() => {
    history.pushState(null, '', window.location.href);
    
    const handlePopState = () => {
      // 1. 열려있는 팝업이나 모달이 있으면 화면 이동 없이 '창만 닫기'
      if (viewerStateRef.current.isOpen) {
      ...
      if (current === 'preview') {
        ...
        setStep(lastStepRef.current);
        history.pushState(null, '', window.location.href);
      } else if (current === 'info') {
        // 첫 화면(info)에서 뒤로가기를 누르면 정상적으로 앱 종료 허용
      }
    };
