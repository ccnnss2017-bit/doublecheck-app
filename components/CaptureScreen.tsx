...
const toStoragePhotos = async (photos: any[]) => {
  return Promise.all(photos.map(async (p) => {
    ...
    if (p && typeof p === 'object' && typeof p.url === 'string' && p.url.startsWith('blob:')) {
      try {
...
export const saveSnapshot = async (info: InspectionInfo, defects: DefectItem[], locations: string[]) => {
  try {
    ...
    // Memory Optimization: Avoid creating massive duplicate Blobs for snapshots.
    // Instead of copying every Blob, we only keep the URLs.
    // NOTE: This means snapshots are only valid during the CURRENT SESSION.
    // We rely on the core DB for persistent save.
    const defectsWithUrlsOnly = defects.map((d) => {
         return {
            ...d,
            farPhotos: d.farPhotos.map(p => typeof p === 'string' ? p : p.url),
            nearPhotos: d.nearPhotos.map(p => typeof p === 'string' ? p : p.url)
         };
    });
...
export const saveToArchive = async (info: InspectionInfo, defects: DefectItem[], locations: string[]) => {
  ...
  // Convert any remaining URLs to Blobs for deep storage.
  const defectsWithBlobs = await Promise.all(
    defects.map(async (d) => {
         return {
            ...d,
